...
public class LogAnalyzerTotal implements Serializable {
    ...
    public void processAccessLogs(String outDir, JavaDStream<ApacheAccessLog> accessLogsDStream) {
        ...
        /**
         * transformToPair传入一个实现Function对象的匿名内部类，目的在于
         * 让用户提供一个RDD到RDD的函数，这个函数会在数据流中的每个批次被调用，生成一个新的流
         * transform（）最常见的应用是重用为RDD写的批处理代码
         * */
        ...
        /**
         * 如果需要跨批次维护状态，针对这种情况，updateStateByKey提供了对一个状态变量的访问，用于键值对形式的DStream;
         * 给定一个由（键，事件）构成的DStream,并传递一个指定如何根据新的事件更新每个键对应状态的函数；
         * 要使用updateStateByKey，提供了一个update(events,oldState)函数，接收与某键相关的事件以及该键之前对应的状态，返回这个键的新状态
         */
         ...
         * 输出结果到SequenceFile
         */
        writableDStream.saveAsHadoopFiles(outDir,
                "pandas",
                Text.class,
                LongWritable.class,
                OutFormat.class);
        ...
        /**
         * foreachRDD可以对RDD进行任何操作，最常见的用例将数据写入mysql数据库；
         * 对于这种操作，spark没有提供saveAs，可以用RDD的eachPartition¡n
         */
        ...
        /**
         * updateStateByKey用来跟踪http各响应代码的计数
         */
    ...
    public LogStatistics getLogStatistics() {
        ...
        return new LogStatistics(
                new Tuple4<>(runningCount.get(), runningSum.get(), runningMin.get(), runningMax.get()),
